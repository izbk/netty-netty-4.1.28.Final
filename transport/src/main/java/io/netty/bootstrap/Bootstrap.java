...
public class Bootstrap extends AbstractBootstrap<Bootstrap, Channel> {
    ...
    public ChannelFuture connect() {
        // 校验handler,group和channelFactory不能为空
        validate();
    ...
    private ChannelFuture doResolveAndConnect(final SocketAddress remoteAddress, final SocketAddress localAddress) {
        // 初始化和注册，返回的对象是一个ChannelFuture对象
        final ChannelFuture regFuture = initAndRegister();
        // ChannelFuture对象中获得了初始化得到的Channel对象
        final Channel channel = regFuture.channel();

        // 判断ChannelFuture对象是否已经完成了，若执行成功则表示注册成功则调用doResolveAndConnect0
        if (regFuture.isDone()) {
            ...
            // 添加一个监听器用来监听操作完成事件operationComplete
            regFuture.addListener(new ChannelFutureListener() {
                ...
                public void operationComplete(ChannelFuture future) throws Exception {
                    ...
                    // 判断若操作完成的时候有异常，则设置PendingRegistrationPromise为失败
                    if (cause != null) {
                        // Registration on the EventLoop failed so fail the ChannelPromise directly to not cause an
                        // IllegalStateException once we try to access the EventLoop of the Channel.
                        promise.setFailure(cause);
                    } else {
                        // 若执行成功，则设置PendingRegistrationPromise为注册成功，最后也会调用doResolveAndConnect0方法
